...
def pluralCheck(phrase):
    ...
    if len(phrase) > 0:
        ...
        if first_letter in list(dictionary.keys()):
            d_index = dictionary[first_letter]
            entries = [word for (word, rule) in d_index]
...
def checkPhrase(index, p_len):
    ...
    if len(phrase) > 0:
        d_index = dictionary[phrase[0].lower()]  # gets the dictionary index where the phrase may be

        for definition in d_index:  # for every definition in that index
            comparator = definition[0]  # the word for that definition
...
def findTypes(word):
    ...
    if first_letter in list(dictionary.keys()):
        index = dictionary[first_letter]
        for entry in index:
            entry_word, word_type = entry[0], entry[1]
    ...
    for i in range(len(words)):  # for each word, find that word's index in the dictionary
        for word, word_type in findTypes(words[i]):
            ...
            if type(word_type) == str:  # for every valid word type known of that word
                ...
                trees[key] = ParseTree.Node(word_type, word)
...
def findRules(rules, phrase_number):
    for parent_rule in rules:
        children = rules[parent_rule]

        for child_rules in children:  # for every set of child rules that can satisfy the parent rule
...
def findComponents(rules):
    ...
    for lower_index, middle_index, upper_index in getSubspans(len(words)):
        if upper_index == -1:  # one-phrase subspans
            for rule in findRules(rules, 1):  # for all one-phrase rules
                ...
                if child_key in trees.keys():
                    ...
                    trees[parent_key] = ParseTree.Node(rule[0], trees[child_key])

        else:  # two-phrase subspans
            for rule in findRules(rules, 2):  # for all two-phrase rules
                ...
                if (child_key1 in trees.keys()) and (child_key2 in trees.keys()):
                    # update the dictionary with that rule
                    trees[parent_key] = ParseTree.Node(rule[0], trees[child_key1], trees[child_key2])


# constructs a dictionary of trees from a given set of rules and returns it
def parse(word_list, d, rules):
    global words, dictionary, trees
    ...
    findComponents(rules)  # adds any possible sentence component to that dictionary (including sentences)
